...
if [ -f "backend/.env.example" ]; then
    cp backend/.env.example portable_dist/backend/.env.template
elif [ -f "backend/.env" ]; then
    cp backend/.env portable_dist/backend/.env.template
fi
...
ENVEOF
        echo "⚙️  Created backend/.env template"
        echo "⚠️  Please update backend/.env with your Azure OpenAI credentials"
    fi
fi
...
if ! ps -p $BACKEND_PID > /dev/null; then
    echo "❌ Backend failed to start. Check backend/.env configuration."
    exit 1
fi
