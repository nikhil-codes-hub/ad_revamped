...
echo "📝 Creating Windows setup.bat..."
cat > portable_dist/setup.bat << 'EOF'
@echo off
echo Setting up AssistedDiscovery for Windows...
echo This will create isolated Python environments for backend and frontend
echo.
...
backend_env\Scripts\pip.exe install -r backend\requirements.txt
if errorlevel 1 (
    echo Failed to install backend dependencies
    pause
    exit /b 1
...
echo Installing frontend dependencies...
frontend_env\Scripts\python.exe -m pip install --upgrade pip
frontend_env\Scripts\pip.exe install -r frontend\requirements.txt
if errorlevel 1 (
    echo Failed to install frontend dependencies
    pause
    exit /b 1
...
REM Create default .env if not exists
if not exist .env (
    if exist .env.template (
        ...
        (
            ...
            echo # Supported providers: azure, gemini
            echo LLM_PROVIDER=azure
            echo.
            echo # Azure OpenAI Configuration
            echo AZURE_OPENAI_ENDPOINT=your-endpoint-here
            echo AZURE_OPENAI_KEY=your-api-key-here
            echo AZURE_API_VERSION=2025-01-01-preview
            echo MODEL_DEPLOYMENT_NAME=gpt-4o
            echo.
            echo # Google Gemini Configuration ^(if using gemini provider^)
            echo GEMINI_API_KEY=your-gemini-api-key-here
            echo GEMINI_MODEL=gemini-1.5-pro
            echo.
            echo # LLM Common Settings
            echo MAX_TOKENS_PER_REQUEST=4000
            echo LLM_TEMPERATURE=0.1
            echo LLM_TOP_P=0.0
            echo.
            echo # Application Settings
            echo LOG_LEVEL=INFO
            echo WORKSPACE_DB_DIR=./data/workspaces
        ) > .env
...
echo "📝 Creating Windows start_app.bat..."
cat > portable_dist/start_app.bat << 'EOF'
@echo off
...
if not exist backend_env (
    echo Virtual environments not found. Please run setup.bat first.
    pause
    exit /b 1
)
...
if not exist .env (
    echo .env not found. Please run setup.bat first.
    pause
    exit /b 1
)
...
start "AssistedDiscovery Backend" cmd /k "..\backend_env\Scripts\uvicorn.exe app.main:app --host 0.0.0.0 --port 8000"
cd ..
...
start "AssistedDiscovery Frontend" cmd /k "..\..\frontend_env\Scripts\streamlit.exe run AssistedDiscovery.py --server.port 8501 --server.headless true"
cd ..\..
...
# Create Windows stop batch file
echo "📝 Creating Windows stop_app.bat..."
cat > portable_dist/stop_app.bat << 'EOF'
@echo off
echo Stopping AssistedDiscovery...

REM Kill backend
taskkill /F /FI "WINDOWTITLE eq AssistedDiscovery Backend*" >nul 2>&1
if not errorlevel 1 echo Backend stopped

REM Kill frontend
taskkill /F /FI "WINDOWTITLE eq AssistedDiscovery Frontend*" >nul 2>&1
