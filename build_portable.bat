...
if exist backend\.env.example (
    copy backend\.env.example portable_dist\backend\.env.template
) else if exist backend\.env (
    copy backend\.env portable_dist\backend\.env.template
)
...
echo         ^(echo LLM_PROVIDER=azure^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo.^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo # Azure OpenAI Configuration^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo AZURE_OPENAI_ENDPOINT=your-endpoint-here^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo AZURE_OPENAI_KEY=your-api-key-here^) ^>^> backend\.env >> portable_dist\setup.bat
...
echo         ^(echo.^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo # LLM Common Settings^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo MAX_TOKENS_PER_REQUEST=4000^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo LLM_TEMPERATURE=0.1^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo LLM_TOP_P=0.0^) ^>^> backend\.env >> portable_dist\setup.bat
echo         ^(echo.^) ^>^> backend\.env >> portable_dist\setup.bat
...
echo echo    1. Update backend\.env with your Azure OpenAI credentials >> portable_dist\setup.bat
...
echo if not exist backend\.env ^( >> portable_dist\start_app.bat
echo     echo ❌ backend\.env not found. Please run setup.bat first. >> portable_dist\start_app.bat
echo     pause >> portable_dist\start_app.bat
echo     exit /b 1 >> portable_dist\start_app.bat
echo ^) >> portable_dist\start_app.bat
echo. >> portable_dist\start_app.bat
